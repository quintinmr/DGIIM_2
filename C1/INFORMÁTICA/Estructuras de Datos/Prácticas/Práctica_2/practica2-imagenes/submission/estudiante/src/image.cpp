...
void Image::set_pixel (int k, byte value) {
    ...
    // Primero, calculamos la columna que ocupa el dato k-ésimo
    int ncol = k % cols;
    // Después calculamos la fila que ocupa el mismo dato
    int nfil = k/cols;

    set_pixel(nfil,ncol,value);
}
...
byte Image::get_pixel (int k) const {
    ...
    // Primero, calculamos la columna que ocupa el dato k-ésimo
    int ncol = k % cols;
    // Después calculamos la fila que ocupa el dato
    int nfil = k/cols;

    return get_pixel(nfil,ncol);
}
